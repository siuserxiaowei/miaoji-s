...
我也整理了一组适合 Codex 新手先装的 10 个研发场景 Skills：前端设计、前端开发、全栈开发、代码审查、网页测试、PR 创建、Lint 修复、文档更新和 Skill 查找。
...
详细说明、截图和公众号文章稿见 [`codex-dev-skills/`](codex-dev-skills/) 与 [`content-ideas/`](content-ideas/)。
...
```bash
claude plugin marketplace add siuserxiaowei/dbskill
...
```bash
npx -y skills add siuserxiaowei/dbskill -g --all
```

#### Trae Solo

Trae Solo 一个 zip 装一个 skill。从 [GitHub Releases](https://github.com/siuserxiaowei/dbskill/releases) 下载最新的 `dbskill-版本号.zip`，解压后里面是 20 个独立的 skill zip（每个 zip 解压后根级是 `SKILL.md`），逐个拖进 Trae Solo 的「上传技能」窗口即可。
